...
cmnext:
	...
	; *******************************************************************
	; CMD_PREV, код 15
	; Переходим на предыдующую дорожку. Байтов ответа нет поэтому игнорируем нулевую длину ответа.
	; Как таковой команды PREV у привода нет, поэтому здесь плеер ориентируясь на каталог диска
	; даёт команду PLAY приводу.
	; Поэтому если нужно переходить на предыдующую дорожку когда воспроизведение остановлено то
	; попреды PREV нужно сразу дать команду PAUSE.
cmprev:
...
4loop:
	...
	; через 4 секунды приостанавливаем воспроизведение и переходим к предыдуще дорожке
	call del2s
	call del2s
	call cmpaus
	call cmprev
